...
@bot.command(name='translate', help='Traduce un texto de español a un idioma a elección. Ejemplo: !translate fr Hola')
async def cumpleaños(ctx):
    message =  ctx.message.content
    print("send a new mesage to rabbitmq: "+message)
    channelMQ.basic_publish(exchange='cartero', routing_key="traductor", body=message)

@bot.command(name='top10', help='Entrega los 10 temas màs escuchados de un artista en spotify. Ejemplo: !top10 Metallica')
async def cumpleaños(ctx):
    message =  ctx.message.content
    print("send a new mesage to rabbitmq: "+message)
    channelMQ.basic_publish(exchange='cartero', routing_key="spotify", body=message)
